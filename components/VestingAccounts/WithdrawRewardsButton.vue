<script lang="tsx" setup>
  ...
  const withdrawRewards = () => {
    ...
    const msg = TransactionMessages.vestingWithdrawRewardsProposal(props.vestingAccountId);

    mutate(msg, {
      onSuccess: () => {
        transactionsStore.setSuccessMessage(() => <SubmitProposalTransactionSuccessful title="Withdraw rewards proposal!" />);
        emit('close');
      },
      onError: () => {
        emit('close');
